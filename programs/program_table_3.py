...
###################################
# generate two-layer tables with 4 vertical legs
# tabletop can be: square, rectangle
# max steps: 6
###################################
...
def generate_single(d):
    ...
    p = np.random.rand()
    if p < 0.5:
        leg_h = 2 * np.random.randint(5, 8) - top_t
    else:
        leg_h = 2 * np.random.randint(4, 11) - top_t
    ...
    layer_r1 = np.random.randint(s1+1, s1+leg_w+1)
    layer_r2 = np.random.randint(s2+1, s2+leg_l+1)
    layer_start = np.random.randint(leg_start, tabletop_start-layer_t)
